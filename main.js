...
function spawnFish() {
    var tempFish = document.createElement('div')
...
function movePlayer(direction) {
    ...
    var currentDiv = $('#player').parent(),
        divId = currentDiv[0].id,
        divPos = getPos(divId),
        newY = Number(divPos[0]),
        newX = Number(divPos[1])
...
function gameStep() {
    if (minutes < 1) {
        ...
        var stars = document.getElementsByClassName('starfish')
        ...
        var fish = document.getElementsByClassName('bigfish')
...
function moveEnemies() {
    //move row shrimps
    var rowClass = document.getElementsByClassName('shrimpRow')

    if (rowClass.length) {
        for (var i = 0; i < rowClass.length; i++) {
            var enemy = rowClass[i].parentElement
            var enemyPos = getPos(enemy.id)
            var move = rowClass[i].name
            var newX = enemyPos[0]
            var newY = Number(enemyPos[1]) + Number(move)
    ...
    //move column shrimps
    var colClass = document.getElementsByClassName('shrimpCol')
    if (colClass.length) {
        for (var i = 0; i < colClass.length; i++) {
            var enemy = colClass[i].parentElement
            var enemyPos = getPos(enemy.id)
            var move = colClass[i].name
            var newX = Number(enemyPos[0]) + Number(move)
            var newY = enemyPos[1]
    ...
    //move snail
    var snails = document.getElementsByClassName('snailA')

    if (snails.length) {
        for (var i = 0; i < snails.length; i++) {
            var parentDiv = snails[i].parentElement
            var oldDiv = getPos(parentDiv.id)

            //snail moves forward the way it is facing
            if (snails[i].name === 1) {
                if (snails[i].getAttribute('face') === 'right') {
                    var newX = Number(oldDiv[1]) + 1
                    var newY = Number(oldDiv[0])
                    ...
                    var newX = Number(oldDiv[1])
                    var newY = Number(oldDiv[0]) - 1
    ...
    //move squids
    var squids = document.getElementsByClassName('squid')

    if (squids.length) {
        for (var i = 0; i < squids.length; i++) {
            var parentDiv = squids[i].parentElement
            var oldDiv = getPos(parentDiv.id)
...
//move sunSpot
function moveSun() {
    var sunSpot = document.getElementsByClassName('sunSpot')

    if (sunSpot.length) {
        var parentDiv = sunSpot[0].parentElement
        var oldDiv = getPos(parentDiv.id)
        var newY = +oldDiv[0] + 1
...
function moveStar() {
    var star = document.getElementsByClassName('starfish')

    if (star.length) {
        var parentDiv = star[0].parentElement.id
        var oldDiv = getPos(parentDiv)
...
function moveBigFish() {
    var bigFish = document.getElementsByClassName('bigfish')
    if (bigFish.length) {
        var parentDiv = bigFish[0].parentElement.id
        var oldDiv = getPos(parentDiv)
...
//squid spawn
function squidSpawn() {
    var tempSquid = document.createElement('div')
    tempSquid.className = 'squid'
    tempSquid.style.background = 'url(./Public/sprites/squid.png) 0px 0px'
    var randPos = Math.floor(Math.random() * (roomSize / 2)) + 1
    var spot = Math.floor(Math.random() * (8))
    switch (spot) {
...
//snail spawn
function snailSpawn() {
    var randPos = Math.floor(Math.random() * (17)) + 1
    var side = Math.floor(Math.random() * (4))
    var tempSnail = document.createElement('div')
...
function foodSpawn() {
    ...
    var x = Math.floor(Math.random() * (roomSize - 2)) + 1
    var y = Math.floor(Math.random() * (roomSize - 2)) + 1

    var tempFood = document.createElement('div')
...
function spriteSnail() {
    ...
    if (snails.length) {
        for (var i = 0; i < snails.length; i++) {
            var facing = snails[i].getAttribute('face')
...
function spriteBigFish() {
    var x = 0 - bigFishOffset
    var bigFish = document.getElementsByClassName('bigfish')

    if (bigFish.length) {
        var facing = bigFish[0].getAttribute('dir')
